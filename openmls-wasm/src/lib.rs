    ...
    pub fn process_message(
        ...
        match msg.into_content() {
            ...
            | openmls::framing::ProcessedMessageContent::ExternalJoinProposalMessage(p) => {
                self.mls_group.store_pending_proposal(provider.0.storage(), *p)?;
                Ok(vec![])
            }
...
mod tests {
    ...
    #[test]
    fn basic() {
        let (mut alice_provider, alice, mut chess_club_alice, mut bob_provider, _, mut chess_club_bob) =
        ...
        assert_eq!(alice_exported_key, bob_exported_key);
